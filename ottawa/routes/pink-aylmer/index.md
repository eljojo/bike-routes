---
name: Pink-Aylmer
status: published
distance_km: 80.1
tags:
- long ride
- bike path
created_at: '2022-10-18'
updated_at: '2024-05-21'
tagline: when pink is not enough and you want poutine
variants:
- name: Pink-Aylmer
  gpx: main.gpx
  distance_km: 80.1
---

This route mixes all the rock stars [Pink Lake](/routes/17-pink-lake-in-gatineau-park) and [Aylmer](/routes/1-aylmer), if you're lucky, splash some [Lac-Leamy](/routes/26-chill-loop-to-lake-leamy) too! What could go wrong!?

It's worth checking [RideGravel.ca's version](https://www.ridegravel.ca/post/loop-de-parc-161-1km) for an even longer edition!
