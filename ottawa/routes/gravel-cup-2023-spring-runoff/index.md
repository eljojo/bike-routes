---
name: 'Gravel Cup: 2023 Spring Runoff '
status: published
distance_km: 110.8
tags:
- gravel
- road
- kinburn
- race
created_at: '2023-04-30'
updated_at: '2023-07-11'
tagline: Epic and Medio editions
variants:
- name: Epic
  gpx: main.gpx
  distance_km: 110.8
- name: Medio
  gpx: variants/medio.gpx
  distance_km: 67.6
---

Kerianne convinced me to join her on the [Gravel Cup series](https://gravelcup.com/), [The Spring Runoff](https://gravelcup.com/gravel-cup-events/ottawa-valley-spring-runoff-2023) was our first race! This route is half road and half gravel, full of beautiful scenery. In early April the white snow beautifully contrasts the dirt and gravel that you'll be speeding through.

We did two "sizes":

- [Medio (~70km)](https://eljojo.bike/rides/543-surprise-spring-runoff-sesh)
- [Epic (~110km)](https://eljojo.bike/rides/549-gravel-cup-2023-spring-runoff)


If you'd like another similar route, check out [The Fall Grally](/routes/20-ottawa-valley-fall-grally).
